...
export default function HomePage({ address, provider }) {
    ...
    try {
      ...
      for (const log of receipt.logs) {
        try {
          ...
          // This log might not be from our contract, ignore it
      ...
      if (tokenId) {
        ...
        // Transaction succeeded but we couldn't parse the tokenId
        console.warn("Could not find Token ID in transaction receipt");
        setWorkflowStatus('minted');
        toast({ 
          title: "Mint Confirmed!", 
          description: "Transaction successful! Check the explorer for token ID.", 
          status: "success",
          duration: 5000,
        });
      }
      ...
      toast({ 
        ...
        description: e.message || "Please check the block explorer for details", 
        status: "error",
        duration: 7000,
      });
    } finally {
  ...
  const handleAnalyzeAndMint = useCallback(async () => {
    ...
    try {
      ...
      const response = await fetch(`${BACKEND_URL}/analyze_and_mint`, { method: 'POST', body: formData });
      ...
      if (!result.txId) {
        throw new Error('No transaction hash received from backend');
      }
